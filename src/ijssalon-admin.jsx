...
export default function AdminApp() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const p = await window.storage.get("ijs-products", true);
      setProducts(p && p.value ? JSON.parse(p.value) : []);
    } catch (_) {
      setProducts([]);
    }
    try {
      const o = await window.storage.get("ijs-orders", true);
  ...
  const persistProducts = async (next) => {
    setProducts(next);
    try {
      await window.storage.set("ijs-products", JSON.stringify(next), true);
    } catch (_) {}
  };
  ...
  const persistOrders = async (next) => {
    setOrders(next);
    try {
      await window.storage.set("ijs-orders", JSON.stringify(next), true);
    } catch (_) {}
  };
...
function ProductForm({ product, onSave, onCancel }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <FormField label="Foto (kleurstijl)">
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {PALETTES.map((pal) => {
            const selected = JSON.stringify(pal.colors) === JSON.stringify(draft.colors);
            return (
              <button
                key={pal.name}
                type="button"
                onClick={() => set("colors", pal.colors)}
                title={pal.name}
                style={{
                  width: 36,
                  height: 36,
                  borderRadius: 10,
                  border: selected ? `2px solid ${COLORS.accent}` : `1px solid ${COLORS.line}`,
                  background: `radial-gradient(circle at 30% 25%, ${pal.colors[0]} 0%, ${pal.colors[1]} 55%, ${pal.colors[2]} 100%)`,
                  cursor: "pointer",
                }}
              />
            );
        ...
        <p style={{ fontSize: 11, color: COLORS.inkSoft, margin: "6px 0 0" }}>
          Dit prototype gebruikt kleurstijlen in plaats van echte productfoto's. Een echte upload vereist bestandsopslag op een server.
        </p>
      </FormField>

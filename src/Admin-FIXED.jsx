...
const WEBAUTHN_STORAGE_KEY = "ijs-admin-webauthn-id";
...
async function registerFingerprint() {
  const cred = await navigator.credentials.create({
    publicKey: {
      ...
      rp: { name: "IJsjes Mauro Beheer" },
      user: { id: randomBytes(16), name: "beheerder", displayName: "Beheerder" },
      pubKeyCredParams: [
  ...
  if (!cred) throw new Error("Geen vingerafdruk geregistreerd.");
  localStorage.setItem(WEBAUTHN_STORAGE_KEY, bufToBase64url(cred.rawId));
}

async function verifyFingerprint() {
  const storedId = localStorage.getItem(WEBAUTHN_STORAGE_KEY);
  if (!storedId) throw new Error("Geen vingerafdruk ingesteld op dit toestel.");
  ...
  if (!assertion) throw new Error("Verificatie mislukt.");
}
...
export default function AdminApp() {
  ...
  const [adminPassword, setAdminPassword] = useState(null); // null = nog aan het laden
  const [hasFingerprint, setHasFingerprint] = useState(false);
  const [showPasswordField, setShowPasswordField] = useState(false);
  const [fpBusy, setFpBusy] = useState(false);
  const [fpError, setFpError] = useState("");
  const [showFpOffer, setShowFpOffer] = useState(false);

  useEffect(() => {
    setHasFingerprint(!!localStorage.getItem(WEBAUTHN_STORAGE_KEY));
    (async () => {
      try {
        const res = await storage.get("admin-password", true);
        setAdminPassword(res && res.value ? res.value : "mauro2026");
      } catch (_) {
        setAdminPassword("mauro2026"); // val terug op standaardwachtwoord als er nog niets is ingesteld
      }
    })();
  }, []);

  const tryLogin = () => {
    if (adminPassword === null) return; // wachtwoord nog niet geladen
    if (passcode === adminPassword) {
      setAuthError("");
      if (webauthnSupported && !hasFingerprint) {
        setShowFpOffer(true); // blijft "niet ingelogd" tot dit scherm is afgehandeld
      } else {
        setAuthed(true);
      }
    } else {
  ...
  const tryFingerprint = async () => {
    ...
    try {
      ...
      setFpError("Vingerafdruk niet herkend. Probeer opnieuw of gebruik je toegangscode.");
    } finally {
      setFpBusy(false);
    }
  };
  ...
  const setupFingerprint = async () => {
    ...
    try {
      ...
      setShowFpOffer(false);
      setAuthed(true);
    } catch (e) {
      setFpError("Instellen mislukt. Werkt je toestel/browser met vingerafdruk-ontgrendeling?");
    } finally {
      setFpBusy(false);
    }
  };
    ...
    return (
      ...
      >
        ...
        >
          ...
          {showFpOffer ? (
            <>
              <p style={{ fontFamily: "Inter, sans-serif", fontSize: 13, color: COLORS.ink, margin: "0 0 16px" }}>
                Volgende keer sneller inloggen met je vingerafdruk?
              </p>
              {fpError && (
                <p style={{ fontFamily: "Inter, sans-serif", fontSize: 12, color: COLORS.accent, margin: "0 0 8px" }}>{fpError}</p>
              )}
              <button
                ...
                style={{
                  ...
                  background: COLORS.accent, color: "#fff", fontFamily: "Inter, sans-serif",
                  fontWeight: 600, fontSize: 15, cursor: fpBusy ? "default" : "pointer", opacity: fpBusy ? 0.6 : 1, marginBottom: 8,
                }}
              >
                {fpBusy ? "Bezig..." : "Ja, vingerafdruk instellen"}
              </button>
              <button
                onClick={() => {
                  setShowFpOffer(false);
                ...
                style={{
                  width: "100%", padding: "12px", borderRadius: 999, border: `1px solid ${COLORS.line}`,
                  background: "none", color: COLORS.inkSoft, fontFamily: "Inter, sans-serif", fontWeight: 600, fontSize: 14, cursor: "pointer",
                }}
              >
                Niet nu
              </button>
            </>
          ) : hasFingerprint && !showPasswordField ? (
            <>
              {fpError && (
                <p style={{ fontFamily: "Inter, sans-serif", fontSize: 12, color: COLORS.accent, margin: "0 0 12px" }}>{fpError}</p>
              )}
              <button
                ...
                style={{
                  width: "100%", padding: "13px", borderRadius: 999, border: "none",
                  background: COLORS.accent, color: "#fff", fontFamily: "Inter, sans-serif",
                  fontWeight: 600, fontSize: 15, cursor: fpBusy ? "default" : "pointer", opacity: fpBusy ? 0.6 : 1,
                  display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                }}
              >
                {fpBusy ? "Bezig..." : "Ontgrendel met vingerafdruk"}
              </button>
              <button
                onClick={() => setShowPasswordField(true)}
                style={{
                  width: "100%", marginTop: 12, background: "none", border: "none",
                  color: COLORS.inkSoft, fontFamily: "Inter, sans-serif", fontSize: 12, textDecoration: "underline", cursor: "pointer",
                }}
              >
            ...
            <>
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 600,
                  fontSize: 15,
                  cursor: adminPassword === null ? "default" : "pointer",
                  opacity: adminPassword === null ? 0.6 : 1,
                }}
              >
                {adminPassword === null ? "Laden..." : "Inloggen"}
              </button>
              {hasFingerprint && (
                <button
          ...
          <p style={{ fontFamily: "Inter, sans-serif", fontSize: 11, color: COLORS.inkSoft, marginTop: 16 }}>
            Wachtwoord vergeten? Pas de waarde bij key <strong>admin-password</strong> aan in de kv_store-tabel in Supabase.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.bg, display: "flex", fontFamily: "Inter, sans-serif" }}>
      ...
      {/* Main */}
      <main className="admin-main" style={{ flex: 1, minWidth: 0, padding: "28px 32px", overflowX: "auto" }}>
        {notice && (
          <div
            style={{
              background: "#0f766e",

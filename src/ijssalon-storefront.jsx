...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const c = await window.storage.get("ijs-cart", false);
  ...
  const persistCart = useCallback(async (next) => {
    setCart(next);
    try {
      await window.storage.set("ijs-cart", JSON.stringify(next), false);
    } catch (_) {}
  }, []);
  ...
  const placeOrder = async () => {
    ...
    try {
      const existing = await window.storage.get("ijs-orders", true);
      const list = existing && existing.value ? JSON.parse(existing.value) : [];
      await window.storage.set("ijs-orders", JSON.stringify([orderRecord, ...list]), true);
    } catch (_) {}
